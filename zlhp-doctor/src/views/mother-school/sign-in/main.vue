<template>
  <div :class="classRoot" v-if="signInData">
    <nav-bar :title="'签到情况'"></nav-bar>
    <div :class="`${classRoot}-body`">
      <van-panel class="sign_up_head">
        <template #header>
          <div class="sign_up_title">{{ signInData.Title }}</div>
          <div class="sign_up_star">
            <star
              :itemTitle="'签到人数'"
              :number="signInData.SignUpCount"
              :numberColor="'#34AE94'"
              :icon="'icon-greenFlowerBG'"
            ></star>
            <star
              :itemTitle="'未报名人数'"
              :number="signInData.SignInCount"
              :numberColor="'#34AE94'"
              :icon="'icon-greenFlowerBG'"
            ></star>
          </div>
      ...
      <van-panel v-if="signInData.SignUpList && signInData.SignUpList.length">
        <template #header>
          <div class="sign_up_table">
            <table>
              ...
              <tr
                class="array_tr"
                v-for="(item, index) in signInData.SignUpList"
                :key="index"
              >
