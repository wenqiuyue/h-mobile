	...
	<div :class="classRoot" v-if="site">
		...
		>
			...
			<ValidationObserver ref="form-1" v-show="step === 1">
				...
				<template
					v-if="
						((site.ArchiveVersion === 1 || site.ArchiveVersion === 3) &&
							form.ContactRelationship === 0) ||
							site.ArchiveVersion === 2 ||
							site.ArchiveVersion === 0
					"
				>
					<ValidationProvider>
						<combobox
					...
					<ValidationProvider>
						<van-field label="乙肝">
							<template #input>
								<van-radio-group v-model="form.HusHBV" direction="horizontal">
									<van-radio
										:name="item.value"
										v-for="(item, index) in options.HusHBV"
										:key="index"
										>{{ item.text }}</van-radio
									>
								</van-radio-group>
			...
			<ValidationObserver
				:ref="
					`form-${
						site.ArchiveVersion === 1 || site.ArchiveVersion === 3 ? 2 : ''
					}`
				"
				v-if="
					(site && site.ArchiveVersion === 1 && step === 2) ||
						(site.ArchiveVersion === 3 && step === 2)
				"
			>
				<ValidationProvider>
				...
				<van-divider>接触有害因素</van-divider>
				<ValidationProvider>
					<van-checkbox-group
				...
				<ValidationProvider>
					<van-field
						class="textarea"
						v-show="form.ExaminationInfo.PhysicalHarmfuls.indexOf(99) !== -1"
						v-model="form.ExaminationInfo.OtherPhysical"
						rows="3"
						autosize
						type="textarea"
						maxlength="50"
						placeholder="其他接触有害因素"
						show-word-limit
					/>
				</ValidationProvider>
			</ValidationObserver>
					...
					<van-field label="吸烟">
						<template #input>
							...
							>
								<van-radio
									:name="item.value"
									v-for="(item, index) in options.Somke"
									:key="index"
									>{{ item.text }}</van-radio
								>
							</van-radio-group>
			...
			<ValidationObserver
				ref="form-3"
				v-if="site && site.ArchiveVersion !== 0 && step === 3"
			>
				<ValidationProvider
					rules="required"
					name="末次月经"
				...
				<ValidationProvider>
					<van-field
						v-show="form.MenstrualHistories.IsMenarcheColor === 0"
						class="textarea"
						v-model="form.MenstrualHistories.MenarcheColorAbnormal"
						rows="3"
						autosize
						type="textarea"
						maxlength="50"
			...
			<ValidationObserver
				ref="form-4"
				v-if="site && site.ArchiveVersion !== 0 && step === 4"
			>
			...
			<ValidationObserver
				ref="form-5"
				v-if="site && site.ArchiveVersion !== 0 && step === 5"
			>
			...
			<ValidationObserver
				ref="form-6"
				v-if="site && site.ArchiveVersion !== 0 && step === 6"
			>
...
export default {
	...
	computed: {
		 ...
		 * 月经周期最小值
		 */
		menstrualCycleMinError() {
			const min = this.form.MenstrualHistories.MenstrualCycleMin;
			const max = this.form.MenstrualHistories.MenstrualCycleMax;
			if (max && min > max) {
				return '不能大于最大值';
			}
			return null;
		 ...
		 * 经期最小值
		 */
		menstrualPeriodMinError() {
			const min = this.form.MenstrualHistories.MenstrualPeriodMin;
			const max = this.form.MenstrualHistories.MenstrualPeriodMax;
			if (max && min > max) {
				return '不能大于最大值';
			}
			return null;
	...
	methods: {
					...
					results.forEach((item, index) => {
						...
						if (index === 2 && item.Code === 0) {
							...
							if (this.site.ArchiveVersion === 0) {
								...
								this.site.ArchiveVersion === 1 ||
								this.site.ArchiveVersion === 3
							) {
								this.stepList = this.stepList.filter((m, ind) => ind !== 2);
							} else if (this.site.ArchiveVersion === 2) {
								this.stepList = this.stepList.filter((m, ind) => ind !== 3);
							}
						}
		...
		parseForm(formObj, reverse) {
			...
			const convertNullString = [
				...
				'OtherHarmfuls',
				'DrugAllergyHistory',
				'MenarcheColorAbnormal'
			];
			// 转空数组
			const convertNullArray = ['Medications'];
			const dispose = obj => {
				Object.keys(obj).forEach(key => {
					...
					if (convertNullArray.indexOf(key) !== -1) {
						if (_.isEmpty(obj[key])) {
							obj[key] = [];
						}
					}
